...
function kv_rebase_url( $url ) {
    ...
    function kv_media_proxy_endpoint() {
        ...
        $safe_name = preg_replace('/[^\x20-\x7E]/', '_', basename($file));
        header('Content-Disposition: inline; filename="' . $safe_name . '"');
        readfile($file);
        exit;
    }
    add_action('wp_ajax_kv_media_proxy',        'kv_media_proxy_endpoint');
    add_action('wp_ajax_nopriv_kv_media_proxy', 'kv_media_proxy_endpoint');
...
// Hide FileOrganizer promotional notice
add_action('admin_head', function () {
    echo '<style>#fileorganizer_promo{display:none!important}</style>';
});
